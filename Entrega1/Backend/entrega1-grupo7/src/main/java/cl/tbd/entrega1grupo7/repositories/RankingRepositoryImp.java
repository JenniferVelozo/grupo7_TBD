...
public class RankingRepositoryImp implements RankingRepository {
    ...
    public void updateRankingByTarea(Integer id_tarea, Integer id_voluntario, Integer flg_participa) {
        try(Connection conn = sql2o.open()){
            //int insertedId = (int) 
            conn.createQuery("UPDATE ranking SET flg_participa = :flg_participa WHERE id_tarea = :id_tarea AND id_voluntario = :id_voluntario" , true)
                    .addParameter("id_tarea", id_tarea)
                    .addParameter("id_voluntario", id_voluntario)
                    .addParameter("flg_participa", flg_participa)
                    .executeUpdate();//.getKey();
            //emergencia.setId(insertedId);        
        }catch(Exception e){
            System.out.println(e.getMessage());
